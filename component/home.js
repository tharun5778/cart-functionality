import React, { Component } from 'react';
import { useRouter } from 'next/router';
import Link from 'next/link';
import axios from 'axios';
import Image from 'next/image';

class Home extends Component{
  constructor(props) {
    super(props);
    this.state = {
      quantity: 0,
      isAdded: false,
    };
  }

  componentWillMount(){
    // productID=this.props.item[0].id;
    if(this.props.cartData.length > 0){
      const cartProduct = this.props.cartData.filter((i)=>{
        return i.productID == this.props.item[0].id;
      })
      if(cartProduct.length == 1){
        this.setState({quantity:cartProduct[0].quantity, isAdded: true})
      }
      
    }
    
  }


  add= async (item)=>{
    // console.log(item);
    // console.log(this.state.quantity)
    if((this.state.isAdded == false) && (this.state.quantity != 0) ){
      // console.log(this.state.isAdded)
      var element = {}
      element.productID = item.id;
      element.quantity = this.state.quantity;
      this.setState({isAdded : true})
      // var a = JSON.stringify(element)
      // console.log(a);
      var config = {
        method: 'post', 
        url: 'http://localhost:4000/cartItems',
        headers: { 
          'Content-Type': 'application/json'
        },
      };
      config.data = element;
      console.log(config);
      // console.log(this.state.isAdded)
  
      axios(config)
      .then(function (response) {
        console.log(JSON.stringify(response.data));
      })
      .catch(function (error) {
        console.log(error);
      });
    }else{
      console.log(this.state.isAdded)
    }

    
  }

  decreaseQuantity= async (item)=>{
    if(this.state.quantity>0){
      if(this.state.isAdded == true){
        var q = this.state.quantity;
        var a = q - 1;
        this.setState({quantity:a});
        var element = {}
        element.productID = item.id;
        element.quantity = a;
        
        var config = {
          method: 'put',
          url: 'http://localhost:4000/cartItems',
          headers: { 
            'Content-Type': 'application/json'
          },
        };
        
        config.data = element;
        axios(config)
        .then(function (response) {
          console.log(JSON.stringify(response.data));
          
        })
        .catch(function (error) {
          console.log(error);
          this.setState({quantity:q});
        });
      }else{
        var q = this.state.quantity - 1;
        this.setState({quantity: q})
      }
      
    } 
  }

  increaseQuantity= async (item)=>{
    if(this.state.quantity<10){
      if(this.state.isAdded == true){
        var q = this.state.quantity;
        var a = q + 1;
        this.setState({quantity:a});
        var element = {}
        element.productID = item.id;
        element.quantity = a;
        
        var config = {
          method: 'put',
          url: 'http://localhost:4000/cartItems',
          headers: { 
            'Content-Type': 'application/json'
          },
        };
        
        config.data = element;
        axios(config)
        .then(function (response) {
          console.log(JSON.stringify(response.data));
          
        })
        .catch(function (error) {
          console.log(error);
          this.setState({quantity:q});
        });
      }else{
        var q = this.state.quantity + 1;
        this.setState({quantity: q})
      }
    }
  }

  render(){
    return (
      <div style={{backgroundColor:'#edebeb',height:'100vh', width:'null'}}>
        {/* <div>
          <nav class="navbar fixed-top navbar-light bg-light">
            <div class="container-fluid">
              <a class="navbar-brand" href="#">Navbar</a>
            </div>
          </nav>
        </div> */}
        <div className="container-md container-lg" >
          <div className="row row-cols-1" >
            <div style={{display: 'flex', justifyContent: 'center'}}>
              <Image
                src="data:image/jpeg;base64,"
                alt="Picture of the author"
                width={300}
                height={300}
              />
            </div>        
        </div>
        <div>
          <div className="row row-cols-1" >
          <div style={{display: 'flex', justifyContent: 'center'}}>
            <div style={{flexDirection:'column'}}>
              <p>{this.props.item[0].productName}</p>
              <p>{this.props.item[0].description}</p>
              <p>Price :  {this.props.item[0].price}</p>
              <div>
                  <Link href={'/cart'}>
                    <button  style={{width: '125px', height: '35px', backgroundColor: "skyblue", borderRadius:'10px'}}>
                      go to cart page
                    </button>
                  </Link>
                </div>
                <div style={{flexDirection:'row'}}>
                  <div>
                    <p> Quantity {"     "}
                      <button style={{width: '40px', height: '35px', backgroundColor: "skyblue", borderRadius:'10px'}} onClick={()=>this.decreaseQuantity(this.props.item[0])}>
                          -
                      </button>
                      {"  "}
                      {this.state.quantity} 
                      {"  "}
                      <button style={{width: '40px', height: '35px', backgroundColor: "skyblue", borderRadius:'10px'}} onClick={()=>this.increaseQuantity(this.props.item[0])}>
                          +
                      </button>
                    </p>
                  </div>
                </div>
                <div>
                  <button style={{width: '100px', height: '35px', backgroundColor: "skyblue", borderRadius:'10px'}} onClick={()=>this.add(this.props.item[0])}>
                      add
                  </button>
                </div>
            </div>
                
                
                
          </div>
          </div>
        </div>
      </div>
    </div>
          
    )
  }
}

export default Home;